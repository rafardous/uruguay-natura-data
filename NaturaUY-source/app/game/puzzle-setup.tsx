...
const SCOPE_KEY = 'puzzle.scope'; const GRID_KEY = 'puzzle.grid'; const LEVEL_KEY = 'puzzle.knowledge_level';
export default function PuzzleSetupScreen(): React.JSX.Element {
  ...
  useEffect(() => { void Promise.all([settingsRepository.get(db, SCOPE_KEY), settingsRepository.get(db, GRID_KEY), settingsRepository.get(db, LEVEL_KEY)]).then(([savedScope, savedGrid, savedLevel]) => { if (savedScope && savedScope in PUZZLE_SCOPES) setScope(savedScope as PuzzleScope); if (savedGrid === '4') setGrid(4); if (savedLevel === 'medium' || savedLevel === 'hard') setLevel(savedLevel); }); }, [db]);
  const start = async () => { await settingsRepository.set(db, SCOPE_KEY, scope); await settingsRepository.set(db, GRID_KEY, String(grid)); await settingsRepository.set(db, LEVEL_KEY, level); router.push({ pathname: '/game/puzzle', params: { scope, grid: String(grid), level } } as never); };
  return <View style={[styles.screen, { backgroundColor: colors.background, paddingTop: insets.top + spacing.sm }]}>
    ...
    <ScrollView contentContainerStyle={{ padding: spacing.lg, gap: spacing.xl }}>
      ...
      <View><Text style={[typography.cardTitle, { color: colors.text }]}>Especies que pueden aparecer</Text><View style={styles.wrap}>{KNOWLEDGE_LEVELS.map((item) => <Chip key={item.id} label={item.label} selected={level === item.id} accent={colors.play} onAccent={colors.onPlay} onPress={() => setLevel(item.id)} />)}</View><Text style={[typography.caption, { color: colors.textMuted, marginTop: 8 }]}>{KNOWLEDGE_LEVELS.find((item) => item.id === level)?.description}</Text></View>
