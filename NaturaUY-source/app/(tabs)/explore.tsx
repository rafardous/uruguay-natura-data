...
import { ActivityIndicator, Modal, Pressable, ScrollView, StyleSheet, Text, View, type NativeScrollEvent, type NativeSyntheticEvent } from 'react-native';
...
import { ChevronRightIcon, CloseIcon, MenuIcon, SlidersIcon, TaxonomyIcon } from '../../src/presentation/components/TabIcons';
...
import { COLLAPSIBLE_HEADER_EXPANDED, NAV_ISLAND_HEIGHT, NAV_ISLAND_MARGIN, spacing as space } from '../../src/presentation/theme/tokens';
...
export default function ExploreScreen(): React.JSX.Element {
  ...
  const canShowResults = search.trim().length >= 2 || count(applied) > 0 || params.all === '1';
  const list = useSpeciesList(filters, canShowResults); const bottom = NAV_ISLAND_HEIGHT + NAV_ISLAND_MARGIN + insets.bottom;
  ...
  const taxonomyCard = <View style={{ marginTop: spacing.md }}><Pressable onPress={() => { haptics.tap(); router.push('/taxonomy'); }}>{({ pressed }) => <LinearGradient colors={['#8A641B', '#6F531A']} style={[styles.taxonomy, elevation.low, { borderRadius: radius.lg, opacity: pressed ? 0.92 : 1 }]}><View style={[styles.taxIcon, { borderRadius: radius.md }]}><TaxonomyIcon color="#FFF9EA" size={25} /></View><View style={styles.flex}><Text style={[typography.cardTitle, { color: '#FFF9EA' }]}>Exploración taxonómica <Text style={styles.taxDash}>— CLASIFICACIÓN DE LA VIDA</Text></Text><Text style={[typography.caption, { color: '#F5E8C4', marginTop: 5 }]}>Recorré cómo se clasifican los seres vivos, de filo a género.</Text></View><ChevronRightIcon color="#FFF9EA" /></LinearGradient>}</Pressable></View>;

  return <View style={[styles.screen, { backgroundColor: colors.background }]}>
    <CollapsibleGradientHeader
      scrollY={scrollY}
      gradient={['#A67A25', '#8A641B', '#5D4515']}
      compactTitle="Descubrir"
      controls={<><Pressable onPress={() => { haptics.tap(); setMenuOpen(true); }} hitSlop={8} accessibilityRole="button" accessibilityLabel="Abrir menú" style={[styles.menuButton, { backgroundColor: 'rgba(255,249,234,0.18)', borderRadius: radius.pill }]}><MenuIcon color="#FFF9EA" /></Pressable><View style={styles.headerTools}><SearchBar value={query} onChange={setQuery} /><AccountButton onPress={() => router.push('/login')} color="#FFF9EA" backgroundColor="rgba(255,249,234,0.16)" /></View></>}
      expandedContent={<View><Text style={[typography.eyebrow, { color: '#F5E8C4' }]}>EXPLORÁ LA NATURALEZA</Text><Text style={[typography.title, { color: '#FFF9EA', marginTop: 4 }]}>Descubrir</Text></View>}
    />
    {!canShowResults ? <Animated.ScrollView onScroll={(event) => { scrollY.value = event.nativeEvent.contentOffset.y; }} scrollEventThrottle={16} contentContainerStyle={{ paddingTop: COLLAPSIBLE_HEADER_EXPANDED, paddingHorizontal: spacing.lg, gap: spacing.md, paddingBottom: bottom }} showsVerticalScrollIndicator={false}>
      {taxonomyCard}
      <Text style={[typography.title, { color: colors.text }]}>Descubrí a tu manera</Text>
      <Text style={[typography.body, { color: colors.textMuted }]}>Buscá una especie o elegí un recorrido para empezar.</Text>
      <Pressable onPress={() => router.push('/species' as never)} style={[styles.featureCard, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg }]}><Text style={[typography.cardTitle, { color: colors.text }]}>Ver todas las especies</Text><Text style={[typography.caption, { color: colors.textMuted }]}>Catálogo completo con filtros y búsqueda</Text><ChevronRightIcon color={colors.primary} /></Pressable>
      <Pressable onPress={() => router.push('/species?native=1' as never)} style={[styles.featureCard, { backgroundColor: colors.surfaceVariant, borderRadius: radius.lg }]}><Text style={[typography.cardTitle, { color: colors.text }]}>Nativas y prioritarias</Text><Text style={[typography.caption, { color: colors.textMuted }]}>Explorá especies destacadas de Uruguay</Text><ChevronRightIcon color={colors.primary} /></Pressable>
      <Pressable disabled style={[styles.featureCard, { backgroundColor: colors.surfaceVariant, borderRadius: radius.lg, opacity: .7 }]} accessibilityRole="button" accessibilityLabel="Aprender, próximamente"><Text style={[typography.cardTitle, { color: colors.text }]}>Aprender</Text><Text style={[typography.caption, { color: colors.textMuted }]}>Taxonomía, estadísticas y curiosidades por nivel · Próximamente</Text></Pressable>
    </Animated.ScrollView> : <>
    <View style={[styles.flex, { paddingTop: COLLAPSIBLE_HEADER_EXPANDED }]}>{taxonomyCard}<View style={[styles.filterRow, { paddingHorizontal: spacing.lg, marginTop: spacing.xl }]}><View><Text style={[typography.label, { color: colors.text }]}>Resultados</Text><Text style={[typography.caption, { color: colors.textMuted }]}>{list.total} especies</Text></View><Pressable onPress={() => { setDraft(applied); setSheetOpen(true); }} style={[styles.filterButton, { backgroundColor: colors.surfaceVariant }]}><SlidersIcon color={colors.textSecondary} /><Text style={[typography.label, { color: colors.textSecondary }]}>Filtros{count(applied) ? ` (${count(applied)})` : ''}</Text></Pressable></View>{count(applied) > 0 && <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.active, { paddingHorizontal: spacing.lg }]}>{applied.onlyNative && <Chip label="Nativas ×" selected onPress={() => remove('onlyNative')} />}{applied.onlyPriority && <Chip label="Prioritarias ×" selected onPress={() => remove('onlyPriority')} />}{(['classes', 'habitats', 'diets', 'seasonalities'] as const).flatMap((key) => applied[key].map((v) => <Chip key={`${key}-${v}`} label={`${friendly(v)} ×`} selected onPress={() => remove(key, v)} />))}</ScrollView>}{list.loading ? <Animated.ScrollView onScroll={(event) => { scrollY.value = event.nativeEvent.contentOffset.y; }} scrollEventThrottle={16} contentContainerStyle={{ padding: spacing.lg, paddingBottom: bottom, gap: spacing.lg }}>{[0,1,2,3].map((i) => <SpeciesCardSkeleton key={i} />)}</Animated.ScrollView> : list.items.length === 0 ? <View style={{ padding: spacing.lg }}><EmptyState title="Sin resultados" message="Probá con otro nombre, o quitá algún filtro." /></View> : <FlashList data={list.items} renderItem={renderItem} keyExtractor={(i) => i.codigo} onScroll={handleListScroll} scrollEventThrottle={32} onEndReached={list.loadMore} onEndReachedThreshold={0.6} ListHeaderComponent={<View style={{ height: spacing.xxl }} />} showsVerticalScrollIndicator={false} ListFooterComponent={list.loadingMore ? <ActivityIndicator color={colors.primary} style={{ paddingBottom: bottom }} /> : <View style={{ height: bottom }} />} />}</View></>}
...
const styles = StyleSheet.create({ screen: { flex: 1 }, flex: { flex: 1 }, menuButton: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center' }, headerTools: { flex: 1, maxWidth: 760, flexDirection: 'row', alignItems: 'center', gap: 10 }, taxonomy: { minHeight: 104, flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14 }, taxIcon: { width: 50, height: 50, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,249,234,.16)' }, taxDash: { fontSize: 12, fontWeight: '700' }, featureCard: { minHeight: 96, padding: 16, justifyContent: 'center', gap: 4, borderWidth: StyleSheet.hairlineWidth }, filterRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, filterButton: { flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 999 }, active: { gap: 8, paddingTop: 10 }, group: { gap: 8 }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, layer: { flex: 1, justifyContent: 'flex-end' }, sheet: { maxHeight: '88%', overflow: 'hidden' }, sheetHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, sheetActions: { flexDirection: 'row', gap: 12, borderTopWidth: StyleSheet.hairlineWidth }, clear: { padding: 14 }, apply: { flex: 1, alignItems: 'center', padding: 15 } });
