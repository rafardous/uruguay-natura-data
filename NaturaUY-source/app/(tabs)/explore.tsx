...
import { ActivityIndicator, Image as NativeImage, Pressable, ScrollView, StyleSheet, Text, View, type NativeScrollEvent, type NativeSyntheticEvent } from 'react-native';
...
import { LearnFieldGuideArt, SpeciesLensArt, TaxonomyMiniMap } from '../../src/presentation/components/DiscoverArtwork';
import { ChevronRightIcon, MenuIcon, SlidersIcon } from '../../src/presentation/components/TabIcons';
...
export default function ExploreScreen(): React.JSX.Element {
  ...
  const [discoverSpecies, setDiscoverSpecies] = useState<Species | null>(null);
  ...
  useEffect(() => {
    let active = true;
    void speciesRepository.count(db, { onlyWithPhoto: true }).then((count) => {
      ...
      return speciesRepository.findPaged(db, { onlyWithPhoto: true }, 1, Number(dateKey) % count);
    }).then((page) => { if (active && page?.items[0]) setDiscoverSpecies(page.items[0]); }).catch(() => undefined);
    return () => { active = false; };
  }, [db]);
  ...
  const taxonomyCard = <View style={{ marginTop: spacing.md }}><Pressable onPress={() => { haptics.tap(); router.push('/taxonomy'); }}>{({ pressed }) => <View style={[styles.taxonomy, elevation.low, { backgroundColor: '#DCEBE0', borderColor: '#B9D3C0', borderRadius: radius.lg, opacity: pressed ? 0.92 : 1 }]}><View style={[styles.taxIcon, { borderRadius: radius.md, backgroundColor: '#C3DCC9' }]}><TaxonomyMiniMap /></View><View style={styles.flex}><Text style={[typography.cardTitle, { color: '#315448' }]}>Exploración taxonómica</Text><Text style={[typography.caption, { color: '#527066', marginTop: 5 }]}>Recorré cómo se clasifican los seres vivos, de filo a género.</Text></View><ChevronRightIcon color="#416A5A" /></View>}</Pressable></View>;
  ...
  return <View style={[styles.screen, { backgroundColor: colors.background }]}>
    <CollapsibleGradientHeader
      scrollY={scrollY}
      gradient={['#3F7A6C', '#34675A', '#274D43']}
      controls={<View style={[styles.topActions, { gap: spacing.sm }]}><Pressable onPress={() => { haptics.tap(); setMenuOpen(true); }} hitSlop={8} accessibilityRole="button" accessibilityLabel="Abrir menú" style={[styles.menuButton, { backgroundColor: 'rgba(255,249,234,0.18)', borderRadius: radius.pill }]}><MenuIcon color="#FFF9EA" /></Pressable><View style={styles.searchWrap}><SearchBar value={query} onChange={setQuery} placeholder="Buscar una especie" collapseOffset={scrollY} /></View><AccountButton onPress={() => router.push('/login')} color="#FFF9EA" backgroundColor="rgba(255,249,234,0.16)" /></View>}
      expandedContent={<Text style={[typography.headerTitle, { color: '#FFF9EA', maxWidth: 350 }]}>Explorá nuestra flora y fauna de distintas maneras</Text>}
    />
...
const styles = StyleSheet.create({ screen: { flex: 1 }, flex: { flex: 1 }, listWrap: { flex: 1 }, refreshIndicator: { position: 'absolute', top: 12, right: 18 }, topActions: { flexDirection: 'row', alignItems: 'center' }, searchWrap: { flex: 1 }, menuButton: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center', borderWidth: StyleSheet.hairlineWidth, borderColor: 'rgba(255,255,255,.2)' }, taxonomy: { minHeight: 112, flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14 }, taxIcon: { width: 68, height: 68, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,249,234,.16)', overflow: 'hidden' }, taxDash: { fontSize: 12, fontWeight: '700' }, featureCard: { minHeight: 104, flexDirection: 'row', alignItems: 'center', padding: 14, gap: 12, borderWidth: StyleSheet.hairlineWidth, overflow: 'hidden' }, featureAccent: { position: 'absolute', left: 0, top: 13, bottom: 13, width: 4, borderTopRightRadius: 4, borderBottomRightRadius: 4 }, featureIcon: { width: 68, height: 68, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }, featureArt: { width: '92%', height: '92%' }, filterRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, filterButton: { flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 999 }, active: { gap: 8, paddingTop: 10 } });
