...
export default function ExploreScreen(): React.JSX.Element {
  ...
  return <View style={[styles.screen, { backgroundColor: colors.background }]}>
    ...
    {!canShowResults ? <Animated.ScrollView onScroll={(event) => { scrollY.value = event.nativeEvent.contentOffset.y; }} scrollEventThrottle={16} contentContainerStyle={{ paddingTop: COLLAPSIBLE_HEADER_EXPANDED + insets.top, paddingHorizontal: spacing.lg, gap: spacing.md, paddingBottom: bottom }} showsVerticalScrollIndicator={false}>
      {taxonomyCard}
      <Pressable onPress={() => { haptics.tap(); router.push('/species' as never); }} style={[styles.featureCard, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg }]}>
        <View style={[styles.featureIcon, { backgroundColor: colors.primaryContainer, borderRadius: radius.md }]}><SearchIcon color={colors.onPrimaryContainer} size={23} /></View><View style={styles.flex}><Text style={[typography.cardTitle, { color: colors.text }]}>Ver todas las especies</Text><Text style={[typography.caption, { color: colors.textMuted, marginTop: 3 }]}>Catálogo completo con filtros y búsqueda</Text></View><ChevronRightIcon color={colors.primary} />
      </Pressable>
      <Pressable disabled style={[styles.featureCard, { backgroundColor: colors.surfaceVariant, borderRadius: radius.lg, opacity: .72 }]} accessibilityRole="button" accessibilityLabel="Aprender, próximamente">
        <View style={[styles.featureIcon, { backgroundColor: colors.surface, borderRadius: radius.md }]}><BookIcon color={colors.textMuted} size={23} /></View><View style={styles.flex}><Text style={[typography.cardTitle, { color: colors.text }]}>Aprender</Text><Text style={[typography.caption, { color: colors.textMuted, marginTop: 3 }]}>Taxonomía, estadísticas y curiosidades · Próximamente</Text></View><Text style={[typography.eyebrow, { color: colors.textMuted }]}>PRÓXIMAMENTE</Text>
      </Pressable>
      <Pressable onPress={() => { haptics.tap(); router.push('/biomes' as never); }} style={[styles.featureCard, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg }]}>
        <View style={[styles.featureIcon, { backgroundColor: '#DDE6D1', borderRadius: radius.md }]}><BiomesIcon color="#4D654A" size={23} /></View><View style={styles.flex}><Text style={[typography.cardTitle, { color: colors.text }]}>Explorar ambientes</Text><Text style={[typography.caption, { color: colors.textMuted, marginTop: 3 }]}>Pastizales, humedales, montes y costas de Uruguay</Text></View><ChevronRightIcon color={colors.primary} />
      </Pressable>
    </Animated.ScrollView> : <>
    {list.loading ? <Animated.ScrollView onScroll={(event) => { scrollY.value = event.nativeEvent.contentOffset.y; }} scrollEventThrottle={16} contentContainerStyle={{ paddingTop: COLLAPSIBLE_HEADER_EXPANDED + insets.top, paddingBottom: bottom }}>{resultsHeader}<View style={{ paddingHorizontal: spacing.lg, gap: spacing.lg }}>{[0,1,2,3].map((i) => <SpeciesCardSkeleton key={i} />)}</View></Animated.ScrollView> : list.items.length === 0 ? <Animated.ScrollView onScroll={(event) => { scrollY.value = event.nativeEvent.contentOffset.y; }} scrollEventThrottle={16} contentContainerStyle={{ paddingTop: COLLAPSIBLE_HEADER_EXPANDED + insets.top, paddingBottom: bottom }}>{resultsHeader}<View style={{ padding: spacing.lg }}><EmptyState title="Sin resultados" message="Probá con otro nombre, o quitá algún filtro." /></View></Animated.ScrollView> : <FlashList data={list.items} renderItem={renderItem} keyExtractor={(i) => i.codigo} onScroll={handleListScroll} scrollEventThrottle={32} onEndReached={list.loadMore} onEndReachedThreshold={0.6} contentContainerStyle={{ paddingTop: COLLAPSIBLE_HEADER_EXPANDED + insets.top }} ListHeaderComponent={resultsHeader} showsVerticalScrollIndicator={false} ListFooterComponent={list.loadingMore ? <ActivityIndicator color={colors.primary} style={{ paddingBottom: bottom }} /> : <View style={{ height: bottom }} />} />}</>}
    <SpeciesFilterSheet
      visible={sheetOpen}
      draft={draft}
