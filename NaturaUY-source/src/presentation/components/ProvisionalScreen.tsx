...
interface ProvisionalScreenProps {
  icon: React.JSX.Element;
  title: string;
  body: string;
}

/** A deliberately quiet placeholder for sections whose full content is next. */
export function ProvisionalScreen({ icon, title, body }: ProvisionalScreenProps): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <View style={[styles.card, elevation.medium, { backgroundColor: colors.surface, borderRadius: radius.xl, padding: spacing.xl }]}>
        <View style={[styles.icon, { backgroundColor: colors.primaryContainer, borderRadius: radius.lg }]}>{icon}</View>
        <Text style={[typography.title, { color: colors.text, marginTop: spacing.lg, textAlign: 'center' }]}>{title}</Text>
        <Text style={[typography.body, { color: colors.textMuted, marginTop: spacing.sm, textAlign: 'center' }]}>{body}</Text>
      </View>
    </View>
  );
}
