...
import { listMedia, listSpecies, requestMediaProcessing, reserveMediaUpload, updateSpeciesAudioMetadata } from '../lib/api';
...
function MediaUploadModal({ species, initialSpeciesId = '', onClose, onComplete }: { species: SpeciesSummary[]; initialSpeciesId?: string; onClose(): void; onComplete(): void }): React.JSX.Element {
  ...
  const [authorizationEvidenceRef, setAuthorizationEvidenceRef] = useState('');
  ...
  async function submit(event: React.FormEvent): Promise<void> {
    ...
    if (kind === 'audio' && author.trim().toLocaleLowerCase('es') !== 'pedro rinaldi') { setError('La primera integración de audio usa exclusivamente grabaciones de Pedro Rinaldi.'); return; }
    if (kind === 'audio' && source.trim().toLocaleLowerCase('es') !== 'xeno-canto') { setError('Para audios usá Xeno-canto como fuente.'); return; }
    ...
    try {
      ...
      if (kind === 'audio') await updateSpeciesAudioMetadata({ mediaId: currentReservation.mediaId, externalId: externalId.trim(), originalLicense: originalLicense.trim(), authorizationEvidenceRef: authorizationEvidenceRef.trim() });
      if (!uploaded) {
  ...
  return <Modal title="Aportar foto o sonido" onClose={onClose}><form className="form-stack" onSubmit={(event) => void submit(event)}>{error && <Notice kind="error">{error}</Notice>}<div className="form-grid two"><label className="field"><span>Especie *</span><select value={speciesId} onChange={(event) => { setSpeciesId(event.target.value); setReservation(null); setUploaded(false); setProgress(0); }} required><option value="">Seleccionar…</option>{species.map((item) => <option value={item.id} key={item.id}>{item.payload.commonNames[0] ?? item.payload.scientificName} — {item.payload.scientificName}</option>)}</select></label><label className="field"><span>Tipo *</span><select value={kind} onChange={(event) => { setKind(event.target.value as 'image' | 'audio'); setSourceFile(null); setUploadFile(null); setClip(null); setPreviewUrl(''); setReservation(null); setUploaded(false); setProgress(0); }}><option value="image">Imagen</option><option value="audio">Audio</option></select></label></div>
    ...
    <div className="form-grid two"><label className="field"><span>Autor o autora *</span><input value={author} onChange={(event) => setAuthor(event.target.value)} required /></label><label className="field"><span>Licencia de Natura UY *</span><select value={license} onChange={(event) => setLicense(event.target.value as MediaAsset['license'])}><option value="CC-BY-4.0">CC BY 4.0</option><option value="CC0">CC0</option><option value="permission">Autorización particular</option></select></label><label className="field"><span>Fuente u origen *</span><input value={source} onChange={(event) => setSource(event.target.value)} required /></label><label className="field"><span>URL de procedencia</span><input type="url" value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} /></label></div>
    {kind === 'audio' && <div className="form-grid two"><label className="field"><span>ID externo (XC) *</span><input value={externalId} onChange={(event) => setExternalId(event.target.value.replace(/^XC/i, ''))} placeholder="123456" required /></label><label className="field"><span>Licencia original *</span><input value={originalLicense} onChange={(event) => setOriginalLicense(event.target.value)} placeholder="CC BY-NC-SA 4.0" required /></label><label className="field"><span>Referencia privada de autorización *</span><input value={authorizationEvidenceRef} onChange={(event) => setAuthorizationEvidenceRef(event.target.value)} placeholder="permiso-pedro-2026-01" required /></label></div>}
