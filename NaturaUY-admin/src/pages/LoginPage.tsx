...
export function LoginPage({ accessDenied = false }: { accessDenied?: boolean }): React.JSX.Element {
  const { signIn, signInWithGoogle, setPassword, resetPassword, passwordFlow } = useAuth();
  const [email, setEmail] = useState(''); const [password, setPasswordValue] = useState(''); const [confirmation, setConfirmation] = useState('');
