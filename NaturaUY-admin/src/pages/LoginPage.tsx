...
export function LoginPage(): React.JSX.Element {
  const { signIn, signInWithGoogle, verifyMfa, setPassword, resetPassword, demo, mfa, passwordFlow } = useAuth();
  ...
  if (passwordFlow) {
    ...
    form = <form className="login-form" onSubmit={(event) => void submit(event)}><p className="eyebrow">PANEL EDITORIAL</p><h2>Iniciar sesión</h2><p>Ingresá con una cuenta que tenga invitación editorial activa.</p>{demo && <Notice>Supabase todavía no está configurado. Podés entrar con cualquier dato para explorar el modo demostración.</Notice>}{message && <Notice kind={message.startsWith('Te enviamos') ? 'success' : 'error'}>{message}</Notice>}{!demo && <button type="button" className="secondary wide" disabled={busy} onClick={() => void signInWithGoogle().then(setMessage)}>Continuar con Google</button>}<label>Correo electrónico<span><Mail size={18} /><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="nombre@correo.com" required={!demo} /></span></label><PasswordField label="Contraseña" value={password} onChange={setPasswordValue} required={!demo} /><button className="primary wide" disabled={busy}>{busy ? 'Ingresando…' : demo ? 'Explorar demostración' : 'Ingresar'}</button><button type="button" className="link-button centered" onClick={() => void reset()}>¿Olvidaste tu contraseña?</button></form>;
  }
